...
public class TrainService : ITrainService
{
	...
	public async Task<IEnumerable<Train>> SearchTrainsAsync(string? departureStation, string? arrivalStation, DateTime? departureDate)
	{
	...
	public async Task<(bool Success, string Message, int TrainId)> CreateTrainAsync(Train train)
	{
		...
		await _auditService.LogAsync(null, "Train Created", "Train", trainId, $"Train {train.TrainNumber} created with {train.TotalSeats} seats.");
	...
	public async Task<(bool Success, string Message)> UpdateTrainAsync(Train train)
	{
		...
		if (success)
		{
			await _auditService.LogAsync(null, "Train Updated", "Train", train.TrainId, $"Train {train.TrainNumber} updated.");
	...
	public async Task<(bool Success, string Message)> DeleteTrainAsync(int trainId)
	{
		...
		if (success)
		{
			await _auditService.LogAsync(null, "Train Deleted", "Train", trainId, $"Train {train.TrainNumber} deleted.");
	...
	public async Task<(bool Success, string Message)> UpdateTrainStatusAsync(int trainId, string status)
	{
		...
		if (success)
		{
			await _auditService.LogAsync(null, "Train Status Updated", "Train", trainId, $"Train {train.TrainNumber} status changed to {status}.");
