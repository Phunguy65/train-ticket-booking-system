...
*   **Dùng khi nào:** Khi người dùng bấm nút "Đăng ký" trên màn hình tạo tài
  khoản.
*   **Input:**
...
*   **Hàm RPC:** `LogoutAsync`
*   **Dùng khi nào:** Khi người dùng bấm nút "Đăng xuất" hoặc muốn thoát tài
  khoản.
*   **Input:** k có
*   **Output:**
...
*   **Input:** Server nhận tham số phân trang, nếu không gửi sẽ trả về toàn bộ
  danh sách (không khuyến khích).
...
*   **Input:** (Sử dụng `UpdateUserRequest` DTO) Gửi các trường muốn cập nhật (các
  trường null sẽ không được thay đổi).
...
*   **Input:** (Sử dụng `LockUnlockUserRequest` DTO) Gửi ID User và trạng thái
  mới:
...
**Yêu cầu trước khi sử dụng:** Đảm bảo bạn đã khởi tạo `ApiClient` và
`TrainService` trước khi gọi hàm:
...
*   **Input:** Server nhận một đối tượng JSON `SearchTrainRequest` (kèm phân
  trang):
...
*   **Dùng khi nào:** Hiển thị danh sách toàn bộ tàu cho Admin quản lý (có phân
  trang).
*   **Input:** tham số phân trang (Optional):
...
*   **Dùng khi nào:** Admin muốn thay đổi nhanh trạng thái (Hủy chuyến, Hoãn, Bảo
  trì...) mà không cần sửa đổi các thông tin khác (giờ chạy, tên tàu...).
*   **Input:** Server nhận ID và Status mới:
...
*   **Dùng khi nào:** Admin bấm nút "Xóa" trên danh sách tàu để loại bỏ một chuyến
  tàu khỏi hệ thống.
*   **Input:** Server nhận ID của tàu cần xóa:
...
Module chịu trách nhiệm hiển thị sơ đồ ghế, xử lý giao dịch đặt vé, hủy vé và
xem lịch sử.
...
*   **Dùng khi nào:** Lấy danh sách trạng thái ghế của một chuyến tàu cụ thể để
  hiển thị lên giao diện chọn chỗ.
...
*   **Output:** danh sách tàu (hoặc đối tượng phân trang `PagedResult` nếu input
  có phân trang):
...
*   **Dùng khi nào:** Hủy một vé đã đặt. Admin có thể hủy vé của bất kỳ ai, User
  chỉ hủy được vé của mình.
*   **Input:**
...
*   **Output:** Đối tượng chứa thông tin chi tiết:
...
*   **Dùng khi nào:** Lấy danh sách tất cả booking trên hệ thống. Hỗ trợ phân
  trang. Chỉ Admin mới gọi được.
