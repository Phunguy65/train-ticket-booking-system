...
namespace client.Forms.TrainSearch
{
	public partial class MainForm : Form
	{
		...
		private void SetupUI()
		{
			...
			{
				Dock = DockStyle.Top, Height = 60, BackColor = ClrHeader, Padding = new Padding(20, 0, 20, 0)
			};
			...
			pnlSearch.Paint += (s, e) =>
			{
				...
				using (GraphicsPath path = GetRoundedPath(new Rectangle(0, 0, pnlSearch.Width, pnlSearch.Height - 20),
					       15))
			...
			pnlSearch.Controls.Add(CreateLabel("Tìm chuyến tàu", 14, FontStyle.Bold, ClrText, xMargin, yPos));
			yPos += 50;

			pnlSearch.Controls.Add(CreateLabel("Ga đi", 10, FontStyle.Regular, ClrTextGray, xMargin, yPos));
			yPos += 30;
			txtDepStation = new ModernTextBox
			{
			...
			pnlSearch.Controls.Add(txtDepStation);
			yPos += 70;

			pnlSearch.Controls.Add(CreateLabel("Ga đến", 10, FontStyle.Regular, ClrTextGray, xMargin, yPos));
			yPos += 30;
			txtArrStation = new ModernTextBox
			{
				Location = new Point(xMargin, yPos),
				Size = new Size(inputW, 45),
			...
			pnlSearch.Controls.Add(txtArrStation);
			yPos += 70;

			pnlSearch.Controls.Add(CreateLabel("Ngày đi", 10, FontStyle.Regular, ClrTextGray, xMargin, yPos));
			yPos += 30;
			txtDate = new ModernTextBox
			{
				Location = new Point(xMargin, yPos),
				Size = new Size(inputW, 45),
			...
			pnlSearch.Controls.Add(txtDate);
			yPos += 80;
			...
			string price, string seatStatus, int statusType)
		{
			...
			pnlItem.Controls.Add(CreateLabel(code, 11, FontStyle.Bold, ClrText, curX, 30));
			curX += colWidths[0];
			pnlItem.Controls.Add(CreateLabel(name, 10, FontStyle.Regular, ClrTextGray, curX, 30));
			curX += colWidths[1];
			pnlItem.Controls.Add(CreateLabel(depTime, 11, FontStyle.Regular, ClrText, curX, 30));
			curX += colWidths[2];
			pnlItem.Controls.Add(CreateLabel(arrTime, 11, FontStyle.Regular, ClrTextGray, curX, 30));
			curX += colWidths[3];
			pnlItem.Controls.Add(CreateLabel(duration, 10, FontStyle.Regular, ClrTextGray, curX, 30));
			curX += colWidths[4];
			pnlItem.Controls.Add(CreateLabel(price, 11, FontStyle.Bold, ClrText, curX, 30));
			curX += colWidths[5];

			Label lblSeat = new Label
			{
				Text = seatStatus,
				ForeColor = statusType == 1 ? ClrGreen : (statusType == 2 ? ClrOrange : ClrRed),
				Font = new Font("Segoe UI", 9, FontStyle.Bold),
				AutoSize = true,
		...
		private void AddWindowControls(Panel parent)
		{
			...
			Label btnClose = CreateWindowButton("✕", startX + (btnW * 2), ClrRed);
			btnClose.Click += (s, e) => Application.Exit();
			btnClose.Anchor = AnchorStyles.Top | AnchorStyles.Right;
			parent.Controls.Add(btnClose);
			Label btnMax = CreateWindowButton("☐", startX + btnW, ClrHeader);
			btnMax.Click += (s, e) => ToggleMaximize();
			btnMax.Anchor = AnchorStyles.Top | AnchorStyles.Right;
			parent.Controls.Add(btnMax);
			Label btnMin = CreateWindowButton("―", startX, ClrHeader);
		...
		private Label CreateWindowButton(string text, int x, Color hoverColor)
		{
			...
			lbl.MouseEnter += (s, e) => lbl.BackColor = (text == "✕") ? ClrRed : Color.FromArgb(51, 65, 85);
		...
		{
			MessageBox.Show($"Tìm kiếm: {txtDepStation.TextValue} -> {txtArrStation.TextValue}", "Đang xử lý");
		}
